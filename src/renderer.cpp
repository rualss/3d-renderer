...
Vec3 TransformVector(const Mat4& transformation_matrix, const Vec3& vector) {
    ...
    assert(homogeneous.w != 0 && "Point went to infinity in TrandformVector");
...
Polygon TransformPolygon(const Mat4& transformation_matrix,
    ...
    Polygon zalupa(transformed_vertices);
    zalupa.SetColor(polygon.GetColor());
    return zalupa;
}
...
void TransformPolygonToScreenSpace(Polygon& polygon, Height height, Width width) {
    ...
    for (Index i = 0; i < Polygon::kVertexCount; ++i) {
        ...
        vertices[i].x = std::clamp(vertices[i].x, 0.f, static_cast<float>(width) - 1.f);
        vertices[i].y = std::clamp(vertices[i].y, 0.f, static_cast<float>(height) - 1.f);
    }
}
...
CoordType GetPolygonZProjection(Vec2 xy, const std::array<Vec3, Polygon::kVertexCount>& vertices,
    ...
    if (std::abs(plane_coefs.z) < kEps) {
        //     std::array<Vec3, Polygon::kVertexCount> vertices_sorted_by_x = vertices;
        //     SortVerticesByX(vertices_sorted_by_x);
        //     if (xy.x < vertices_sorted_by_x[1].x) {
        //         return min();
        //     }
        assert(false);
    }
...
void DrawPolygon(Picture& picture, const Polygon& polygon) {
    ...
    // TODO check if polygon is perpendicular to Oxy
